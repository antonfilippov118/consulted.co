...
describe UpdatesOrCreatesAvailability do
  ...
  it 'should create a new availability when requested without an id' do
    ...
    result = action.for user, start: (Time.now.to_i * 1000), end: ((Time.now + 60.minutes).to_i * 1000)
  ...
  it 'should update an existing availability' do
    ...
    result = action.for user, id: id, start: (Time.now.to_i * 1000), end: ((Time.now + 60.minutes).to_i * 1000)
    expect(result.success?).to be_true
    expect(user.availabilities.count).to eql 1
  end
  ...
  it 'should create a new availability when requested with a false id' do
    user = User.create valid_params
    result = action.for user, id: 'foo', start: (Time.now.to_i * 1000), end: ((Time.now + 60.minutes).to_i * 1000)
    expect(result.success?).to be_true
    expect(user.availabilities.count).to eql 1
  end
  ...
  it 'should update an existing availability with the times given' do
    ...
    result = action.for user, id: id, start: ((start - 10.minutes).to_i * 1000), end: ((ending + 10.minutes).to_i * 1000)
