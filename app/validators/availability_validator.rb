class AvailabilityValidator < ActiveModel::Validator
  ...
  class CountValidator
    ...
    def self.too_many?(time)
      recurring       = time.class.for_user(time.user).recurring.count
      times_this_week = time.class.for_user(time.user).in_week(time.week_of_year).count
      if times_this_week + recurring > maximum_per_week
        time.errors.add :base, 'This week already has the maximum of times possible.'
        return true
      end
      false
    end
    ...
    def self.too_many_a_day?(time)
      minutes_today     = time.class.for_user(time.user).on(time.weekday).in_week(time.week_of_year).sum(&:length)
      minutes_recurring = time.class.recurring.for_user(time.user).on(time.weekday).sum(&:length)
  ...
  class IntervalValidator
    ...
    def self.times_to_check_for(time)
      times_today     = time.class.for_user(time.user).on(time.weekday).in_week(time.week_of_year)
      times_recurring = time.class.for_user(time.user).on(time.weekday).recurring
